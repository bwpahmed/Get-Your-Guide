...
Load and follow the pinned upstream capability at `.agents/vendor/spec-kit/templates/commands/implement.md` only when relevant to the current task. Inspect references/scripts in that vendor subtree before using them. Do not create parallel project logic or bypass repository safety rules.
