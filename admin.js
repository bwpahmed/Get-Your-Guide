...
function openEditor(type, item) {
  ...
  $('#item-form').onsubmit = (event) => { event.preventDefault(); const form = new FormData(event.target); const config = typeConfig(type); const next = { ...draft }; for (const [key, value] of form.entries()) { if (key === 'addon') continue; if (['originalPrice','offerPrice','childPrice','price'].includes(key)) next[key] = Number(value); else next[key] = value; } ['visible','featured'].forEach((key) => { if (key in next) next[key] = form.has(key); }); ['highlights','landmarks','inclusions','exclusions','badges','notes'].forEach((key) => { if (key in next) next[key] = lines(form.get(key)); }); if (type === 'package') next.addOnIds = form.getAll('addon'); const list = data[config.key]; const index = list.findIndex((entry) => entry.id === next.id); if (index >= 0) list[index] = next; else list.push(next); $('#drawer').innerHTML = ''; render(); };
}
